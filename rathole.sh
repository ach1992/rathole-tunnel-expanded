...
download_and_extract_rathole() {
    ...
    if [[ "$ARCH" == "x86_64" ]]; then
    	DOWNLOAD_URL='https://github.com/ach1992/rathole-tunnel-expanded/raw/main/core/rathole.zip'
    fi
...
# Get server IP (needed for ipwhois lookup)
SERVER_IP=$(hostname -I | awk '{print $1}')
if [[ -z "$SERVER_IP" ]]; then
  SERVER_IP=$(curl --max-time 3 -sS https://api.ipify.org 2>/dev/null)
fi
...
read_keepalive_settings() {
  local ks ki

  echo
  colorize cyan "TCP Keepalive settings (press Enter to use defaults)" bold

  # keepalive_secs
  while true; do
    echo -ne "[*] keepalive_secs (default: ${KEEPALIVE_SECS_DEFAULT}): "
    read -r ks
    if [[ -z "$ks" ]]; then
      KEEPALIVE_SECS="$KEEPALIVE_SECS_DEFAULT"
      break
    fi
    if [[ "$ks" =~ ^[0-9]+$ ]] && (( ks >= 1 && ks <= 86400 )); then
      KEEPALIVE_SECS="$ks"
      break
    fi
    colorize red "Invalid number. Enter 1..86400 or press Enter."
  done

  # keepalive_interval
  while true; do
    echo -ne "[*] keepalive_interval (default: ${KEEPALIVE_INTERVAL_DEFAULT}): "
    read -r ki
    if [[ -z "$ki" ]]; then
      KEEPALIVE_INTERVAL="$KEEPALIVE_INTERVAL_DEFAULT"
      break
    fi
    if [[ "$ki" =~ ^[0-9]+$ ]] && (( ki >= 1 && ki <= 3600 )); then
      KEEPALIVE_INTERVAL="$ki"
      break
    fi
    colorize red "Invalid number. Enter 1..3600 or press Enter."
  done

  # sanity: interval should not exceed secs
  if (( KEEPALIVE_INTERVAL > KEEPALIVE_SECS )); then
    colorize yellow "keepalive_interval > keepalive_secs; setting interval=secs (${KEEPALIVE_SECS})."
...
iran_server_configuration() {  
	...
	# Add IPv6 Support (STRICT y/n)
	local_ip="0.0.0.0"
	while true; do
		echo -ne "[-] Listen for IPv6 address? (y/n): "
		read -r answer
		if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
			...
			colorize red "Invalid input. Please enter 'y' or 'n'."
		fi
	done
	...
	# Initialize keepalive defaults + prompt user (required)
	KEEPALIVE_SECS="$KEEPALIVE_SECS_DEFAULT"
	KEEPALIVE_INTERVAL="$KEEPALIVE_INTERVAL_DEFAULT"
	read_keepalive_settings
...
EOF
	...
	prompt_healthcheck_enable "rathole-kharej${tunnel_port}.service" "y"
}
...
tunnel_management() {
	...
	colorize reset "8) Enable healthcheck"
	colorize reset "9) Disable healthcheck"
    ...
    case $choice in
		...
		8) add_healthcheck "$service_name" ;;
		9) remove_healthcheck "$service_name" ;;
        0) return 1 ;;
...
prompt_healthcheck_enable() {
  ...
  while true; do
    echo
    echo -ne "[-] Enable healthcheck for this tunnel? (y/n): "
    read -r ans
    if [[ "$ans" == "y" || "$ans" == "Y" ]]; then
      add_healthcheck "$service_name"
      return 0
    elif [[ "$ans" == "n" || "$ans" == "N" ]]; then
      colorize yellow "Healthcheck not enabled." bold
      sleep 1
      return 0
    else
      colorize red "Invalid input. Please enter 'y' or 'n'."
...
SCRIPT_URL="https://github.com/ach1992/rathole-tunnel-expanded/raw/main/rathole.sh"
...
install_modified_core(){
	echo
	DOWNLOAD_URL='https://github.com/ach1992/rathole-tunnel-expanded/raw/main/core/rathole_modified.zip'
...
read_option() {
  ...
  while true; do
    ...
    case "$choice" in
      1) configure_tunnel; break ;;
      2) tunnel_management; break ;;
      3) check_tunnel_status; break ;;
      4) hawshemi_script; break ;;
      5) download_and_extract_rathole "sleep"; break ;;
      6) update_script; break ;;
      7) change_core; break ;;
      8) remove_core; break ;;
      0) exit 0 ;;
      *) colorize red "Invalid input. Please enter a number between 0 and 8." bold ;;
    esac
  done
}
